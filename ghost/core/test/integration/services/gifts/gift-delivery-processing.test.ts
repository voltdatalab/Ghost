...
const SendGiftDeliveryEvent = require('../../../../core/server/services/gifts/events/send-gift-delivery-event');
...
describe('Gift delivery processing', function () {
    ...
    it('claims, sends, and marks only the requested delivery as sent', async function () {
        ...
        assert.deepEqual(deliverySend.firstCall.args[1], {[requested.delivery.get('recipient_email')]: {}});

        const requestedReloaded = await models.GiftDelivery.findOne({gift_id: requested.gift.id}, {require: true});
        ...
        const untouchedReloaded = await models.GiftDelivery.findOne({gift_id: untouched.gift.id}, {require: true});
