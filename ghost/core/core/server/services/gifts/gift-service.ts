...
export class GiftService {
    ...
    async completePurchase(input: GiftPurchaseData): Promise<boolean> {
        ...
        await this.deps.giftRepository.transaction(async (transacting) => {
            ...
            if (data.deliveryMethod === 'email') {
                ...
                await this.deps.giftDeliveryRepository.create(delivery, {transacting});
            }
        });

        if (deliveryId) {
            this.deps.dispatchGiftDelivery(deliveryId);
        }
    ...
    async sendDelivery(id: string): Promise<'sent' | 'skipped' | 'failed'> {
